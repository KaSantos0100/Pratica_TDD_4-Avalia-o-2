...
class Login_OK_Test(TestCase):
    def setUp(self):
        ...
        new_user.set_password('alunofatec')
        new_user.save()
        data = {'email':'aluno.fatec@fatec.sp.gov.br', 'password':'alunofatec'}
...
class Login_Fail_Test(TestCase):
    def setUp(self):
        ...
        new_user.save()
        data = {'username':'admin', 'password':'alunofatec'}
        self.resp = self.client.post(r('login'), data)
...
class Logout_Get_OK_Test(TestCase):
    def setUp(self):
        ...
        new_user.set_password('alunofatec')
        new_user.save()
        self.client.login(username="admin", password="alunofatec")
